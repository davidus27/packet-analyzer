...
bool ProcesedPacket::is_using(const std::string &protocol) const
{
    ...
    return protocol == "TFTP" || protocol == this->application_protocol || this->ether_type.find(protocol) != std::string::npos || this->transport_protocol == protocol;
}
...
void ProcesedPacket::set_ports(const uint8_t *transport_data_start, const std::vector<std::pair<int, std::string>> &configuration)
{
    ...
    if (!this->src_port)
        this->src_port = big_endian_to_small(*(uint16_t *)transport_data_start);
    if (!this->dst_port)
        this->dst_port = big_endian_to_small(*(uint16_t *)(transport_data_start + 2));
}

void ProcesedPacket::set_arp_flags()
{
    if (this->ether_type == "ARP-REQUEST")
        this->is_starting_packet = true;
    else if (this->ether_type == "ARP-REPLY")
        this->is_ending_packet = true;
}
